...
describe("getFirstCharacters", function() {
  it("should return empty array when contents is an empty string", function() {
    let contents = "";
    assert.deepEqual(getFirstCharacters(contents, 1), []);
  });

  it("should return an empty array when the number of characters is 0", function() {
    let contents = "a";
    assert.deepEqual(getFirstCharacters(contents, 0), []);
  });

  it("should return an array with 1 element when the number of characters is 1", function() {
    let contents = "ab";
    assert.deepEqual(getFirstCharacters(contents, 1), ["a"]);
  });

  it("should return an array with 2 elements when the number of characters is 3 and the conten has a length of 2 only", function() {
    let contents = "ab";
    assert.deepEqual(getFirstCharacters(contents, 3), ["a", "b"]);
  });
});
